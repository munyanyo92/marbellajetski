...
# -*- coding: utf-8 -*-
"""
Generate Marbella JetSki Parking Service PDF (ES) — Premium design.
"""
...
class ParkingPDF(FPDF):
    ...
    def build_page1(self):
        ...
        self.set_xy(0, 112)
        self.cell(pw, 7, "Desde 1998  \u00b7  Costa del Sol  \u00b7  Marbella", align='C')

        # ── TITLE SECTION ──
        self.set_font("Arial", "B", 12)
        self.set_text_color(*self.navy)
        self.set_xy(0, 154)
        self.cell(pw, 14, "SERVICIO DE PARKING DE MOTOS DE AGUA", align='C')
        ...
        self.set_text_color(*self.gold_light)
        self.set_xy(0, 173.5)
        self.cell(pw, 9, "S E R V I C I O   P R E M I U M   \u00b7   T O D O   E L   A \u00d1 O", align='C')

        # Subtitle desc
        self.set_font("Arial", "", 5.5)
        self.set_text_color(*self.slate)
        self.set_xy(0, 192)
        self.cell(pw, 7, "Instalaciones privadas y seguras en primera l\u00ednea de playa  \u00b7  Marbella, Costa del Sol", align='C')
        ...
        self.set_text_color(*self.navy)
        self.set_xy(m + 24, y)
        self.cell(200, 10, "QU\u00c9 INCLUYE")

        items = [
            ("Parking anual o mensual", "Plaza asignada en instalaciones privadas con acceso controlado y vallado perimetral"),
            ("Seguridad las 24 horas", "Vigilante nocturno permanente + sistema completo de videovigilancia CCTV en todas las zonas"),
            ("Limpieza exterior profesional", "Lavado con agua dulce, aclarado y secado completo despu\u00e9s de cada salida al mar"),
            ("Desalinizaci\u00f3n del motor tras cada uso", "Enjuague completo del circuito de agua y tratamiento anticorrosi\u00f3n para proteger los componentes internos"),
            ("Servicio de botadura y recogida", "Nosotros llevamos tu moto al agua y la recogemos \u2014 t\u00fa solo llegas y disfrutas"),
            ("Transporte incluido", "Recogida de tu moto hasta 50 km gratis. A partir de ah\u00ed: 1 \u20ac/km (m\u00e1x. 200 km en total)"),
        ]
        ...
        self.cell(300, 7, "EL CLIENTE DEBE PROPORCIONAR")
        self.set_font("Arial", "B", 5.8)
        self.set_text_color(100, 65, 10)
        self.set_xy(m + 36, ay + 12)
        self.cell(400, 6, "Funda protectora para la moto de agua (obligatorio)")
        self.set_font("Arial", "", 4.8)
        self.set_text_color(120, 85, 20)
        self.set_xy(m + 36, ay + 19)
        self.multi_cell(pw - 2 * m - 46, 5,
            "Tu moto debe estar cubierta en todo momento. La funda protege del sol, lluvia, polvo y corrosi\u00f3n. No se aceptan motos sin funda. \u00bfNecesitas una? Cons\u00faltanos.")
        ...
        boxes_data = [
            ("ACCESO Y HORARIOS", [
                "Horario: 11:00 \u2013 20:00 (15 May \u2013 1 Oct)",
                "Reserva tu salida por tel\u00e9fono o WhatsApp",
                "Aviso m\u00ednimo de 2 horas para botadura",
            ]),
            ("SERVICIO DE BOTADURA", [
                "Transportamos tu moto hasta la orilla del mar",
                "Botadura y recogida profesional",
                "Lavado y desalinizaci\u00f3n post-salida",
                "No necesitas remolque ni veh\u00edculo propio",
            ]),
            ("ALMACENAMIENTO INVERNAL", [
                "Almacenamiento todo el a\u00f1o",
                "Servicio de invernaje opcional disponible",
                "Tu moto lista cuando llegue la primavera",
            ]),
            ("TRANSPORTE (SOLO PREMIUM ANUAL)", [
                "Recogida y entrega incluidas hasta 50 km",
                "A partir de 50 km: 1 \u20ac/km adicional",
                "Distancia m\u00e1xima: 200 km en total",
            ]),
        ]
        ...
        self.set_text_color(180, 195, 215)
        self.set_xy(0, fy + 6)
        self.cell(pw, 6, "+34 655 442 232  \u00b7  marbellajetski.com  \u00b7  WhatsApp disponible", align='C')
    ...
    def build_page2(self):
        ...
        self.set_text_color(*self.navy)
        self.set_xy(m, 38)
        self.cell(200, 12, "NUESTRAS TARIFAS")
        ...
        self.set_text_color(*self.cyan_dark)
        self.set_xy(0, 60)
        self.cell(pw, 8, "S E R V I C I O   P R E M I U M   \u00b7   T O D O   E L   A \u00d1 O", align='C')
        ...
        cards = [
            {
                "badge": "M E J O R   V A L O R",
                "price": "3.000\u20ac",
                "name": "PREMIUM ANUAL",
                "period": "por a\u00f1o",
                "features": [
                    "Almacenamiento todo el a\u00f1o",
                    "Transporte hasta 50 km incluido",
                    "+1\u20ac/km extra (m\u00e1x. 200 km)",
                    "1 transporte almac\u00e9n \u2192 playa incl.",
                    "Estancia en playa todo el verano",
                    "Protecci\u00f3n 24/7 + vigilante",
                    "Limpieza + desalinizaci\u00f3n incl.",
                    "Mantenimiento: precios competitivos",
                    "Aviso 2 semanas antes de entrega",
                    "Temporada: 15 May \u2013 1 Oct",
                ],
                "hl": True,
            },
            {
            ...
            },
            {
                ...
                "price": "800\u20ac",
                "name": "MENSUAL PLAYA",
                "period": "por mes (May\u2013Oct)",
                "features": [
                    "Disponible 15 May \u2013 1 Oct",
                    "Plaza en playa con protecci\u00f3n",
                    "Vigilancia 24/7 + guardia",
                    "Limpieza + desalinizaci\u00f3n incl.",
                    "Servicio de botadura y recogida",
                    "Fuera de temporada: 199\u20ac/mes",
                    "(almacenamiento solo, Oct\u2013May)",
                ],
                "hl": False,
            },
        ]
        ...
        self.set_text_color(*self.slate)
        self.set_xy(0, ny)
        self.cell(pw, 6, "IVA incluido  \u00b7  Pago por adelantado  \u00b7  Consultar condiciones de cancelaci\u00f3n", align='C')
        ...
        self.set_text_color(*self.navy)
        self.set_xy(m, sy)
        self.cell(200, 10, "SERVICIOS OPCIONALES")
        self.set_fill_color(*self.gold)
        ...
        self.cell(300, 6, "SERVICIO")
        self.set_xy(pw - m - 75, thy + 4)
        self.cell(63, 6, "PRECIO", align='R')
        # Header line
        ...
        services = [
            "Invernaje completo (motor, combustible, bater\u00eda y preparaci\u00f3n)",
            "Des-invernaje y revisi\u00f3n de puesta a punto primavera",
            "Cambio de aceite y filtro",
            "Limpieza interior profunda y detailing",
            "Sustituci\u00f3n de bater\u00eda",
            "Reparaciones mec\u00e1nicas menores y diagn\u00f3stico",
            "Funda protectora (suministro e instalaci\u00f3n)",
        ]
        ...
        for idx, svc in enumerate(services):
            ...
            self.cell(63, 6, "A consultar", align='R')
            ry += 11.5
        ...
        self.cell(300, 7, "RESPONSABILIDAD Y CONDICIONES")
        ...
        self.multi_cell((pw - 2 * m - 32), 5.2,
            "El cliente es responsable de mantener su propio seguro de moto de agua (terceros y todo riesgo). Marbella Jet Ski Watersports no se responsabiliza de robos, vandalismo, da\u00f1os meteorol\u00f3gicos ni aver\u00edas mec\u00e1nicas. Se podr\u00e1 solicitar justificante de seguro.")
        ...
        self.multi_cell((pw - 2 * m - 32), 5.2,
            "Contrato firmado obligatorio. M\u00ednimo: 1 mes. Anuales: pago adelantado. Mensuales: d\u00eda 1. Impago >15 d\u00edas: restricci\u00f3n de acceso. Motos no reclamadas tras 60 d\u00edas sujetas a retenci\u00f3n (art. 1780 CC). Condiciones completas bajo solicitud.")
        ...
        self.cell(pw, 4, "\u00a9 2026 Marbella Jet Ski Watersports  \u00b7  Todos los derechos reservados  \u00b7  Desde 1998", align='C')
...
pdf.build_page1()
pdf.build_page2()
out = r"c:\Users\Usuario\Website Marbella Jetski\assets\media\Marbella_JetSki_Parking_Service_ES.pdf"
pdf.output(out)
